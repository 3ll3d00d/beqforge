...
def _channel(channel) -> dict[str, Any]:
    return {
        ...
        "share_se": _arr(channel.share_se),
        "tracking": _arr(channel.tracking),
        "level_spread_db": _arr(channel.level_spread_db),
        "contrast_db": _arr(channel.contrast_db),
        "contrast_se_db": _arr(channel.contrast_se_db),
...
def write(
    ...
    document = {
        ...
        "evidence_contract": {
            "version": "conditional-temporal-v1",
            "mastering_rolloff_support": None,
            "correction_score": "boost-weighted relative precision of temporal contrast; uncalibrated",
            "preference_shaping": "plateau-relative target; original source spectrum unknown",
        },
